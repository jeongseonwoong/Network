...
CEthernetLayer::CEthernetLayer(char* pName) //생성자 및 헤더 초기화 
	: CBaseLayer(pName)
{
	ResetEthernetHeader();
}
...
{
	memset(m_etherNetHeader.destination_address.addrs, 0, 6); //출발지 주소 0
	memset(m_etherNetHeader.source_address.addrs, 0, 6); //목적지 주소 0
	memset(m_etherNetHeader.total_data, 0, ETHER_MAX_DATA_SIZE);
	m_etherNetHeader.frame_type = 0x3412; // 0x0800 프레임 타입 설정
}
